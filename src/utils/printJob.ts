...
export const buildJobVoucherDoc = async (jobId: string): Promise<{ doc: jsPDF; fileName: string }> => {
  ...
  const [
    ...
    supabase.from('t_trabajo_productos').select('*, t_productos(nombre)').eq('trabajo_id', jobId),
  ...
  const isPresupuesto = job.estado === 'PRESUPUESTADO';
  ...
  doc.setFontSize(14);
  doc.text(isPresupuesto ? 'PRESUPUESTO' : 'COMPROBANTE DE TRABAJO', 120, 20);
  doc.setFontSize(10);
  if (isPresupuesto) {
    doc.text(`Fecha Emisión: ${new Date().toLocaleDateString('es-AR')}`, 120, 27);
  } else {
  ...
  const tableData = (items || []).map(item => [
    item.nombre || (item as any).t_productos?.nombre || 'Producto',
    item.cantidad,
  ...
  doc.text(
    isPresupuesto
      ? 'Este documento es un presupuesto sujeto a cambios, no válido como factura fiscal ni comprobante de pago.'
      : 'Este documento es un comprobante de trabajo y estado de cuenta, no válido como factura fiscal.',
    15,
    280
  );
  ...
  const fileName = `${isPresupuesto ? 'Presupuesto' : 'Trabajo'}_${jobId.slice(0, 8)}_${client?.razon_social || 'cliente'}.pdf`;
...
export const printJobVoucher = async (jobId: string) => {
  try {
    const { doc, fileName } = await buildJobVoucherDoc(jobId);
    doc.save(fileName);
