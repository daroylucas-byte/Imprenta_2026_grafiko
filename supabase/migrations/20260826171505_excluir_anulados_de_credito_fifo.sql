-- aplicar_credito_a_trabajos podía repartir crédito FIFO contra un trabajo ANULADO/CANCELADO
-- (v_saldo_trabajos no filtra por estado). Se agrega el filtro explícito.
...
COMMENT ON FUNCTION aplicar_credito_a_trabajos IS 'Reparte FIFO el crédito disponible (recibos sin aplicar) de un cliente contra trabajos con saldo pendiente, excluyendo trabajos ANULADO/CANCELADO. Si p_trabajo_id es NULL aplica contra todos los trabajos pendientes del cliente (uso desde ClientLedgerModal); si se pasa un p_trabajo_id, solo contra ese trabajo (uso desde JobModal).';
